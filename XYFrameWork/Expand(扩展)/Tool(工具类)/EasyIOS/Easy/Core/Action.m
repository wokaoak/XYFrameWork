...
-(NSURLSessionDownloadTask *)Download:(Request *)msg{
    ...
    NSURLSessionDownloadTask *op = [manager downloadTaskWithRequest:request progress:nil destination:^NSURL *(NSURL *targetPath, NSURLResponse *response) {
...
-(NSURLSessionDataTask *)Upload:(Request *)msg{
    ...
    @weakify(msg,self);
    NSURLSessionDataTask *op = [manager uploadTaskWithStreamedRequest:request progress:nil completionHandler:^(NSURLResponse * response, NSDictionary*responseObject, NSError * error) {
        @strongify(msg,self);
        if (error == nil) {
            msg.output = responseObject;
            [self checkCode:msg];
